...
@app.post("/api/profiles/{profile_id}/metadata-refresh", response_model=JobCreated, dependencies=[Depends(require_admin)])
async def refresh_metadata(profile_id: str, payload: MetadataRefreshCreate) -> dict:
    try:
        job_id = await manager.create_metadata_refresh(profile_id, payload.aweme_ids)
        return {"job_id": job_id, "status": "queued"}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Profile not found") from exc
    except ValueError as exc:
...
@app.post("/api/profiles/{profile_id}/metadata-refresh/retry", response_model=JobCreated, dependencies=[Depends(require_admin)])
async def retry_metadata(profile_id: str, payload: RetryCreate) -> dict:
    try:
        job_id = await manager.retry_metadata(profile_id, payload.aweme_ids)
        return {"job_id": job_id, "status": "queued"}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Profile not found") from exc
    except ValueError as exc:
...
@app.get("/api/profiles/{profile_id}/files/{file_id:path}", dependencies=[Depends(require_admin)])
async def profile_file(profile_id: str, file_id: str) -> FileResponse:
    try:
        path, name = manager.profile_file(profile_id, file_id)
    except (KeyError, FileNotFoundError) as exc:
        raise HTTPException(status_code=404, detail="File not found") from exc
    return FileResponse(path, filename=name, media_type=mimetypes.guess_type(path.name)[0] or "application/octet-stream")
