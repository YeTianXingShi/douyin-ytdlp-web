...
- 本项目是在 yt-dlp 基础上开发的 Web 界面；Web 界面层负责用户交互、任务编排和结果展示，yt-dlp 负责视频信息提取与下载能力。
- 需要判断下载能力、支持的站点、URL 形式、Cookie 参数或格式选择时，先确认底层 yt-dlp 源码，再设计或修改 Web 界面行为；不要让 Web 界面承诺底层提取器未实现的能力。
...
- 本工作区的 yt-dlp 源码位于 `yt-dlp/`；涉及 yt-dlp 行为、站点支持、命令行参数或提取器能力的判断，先查看该目录中的源码，再下结论。
- 重点查看 `yt-dlp/yt_dlp/` 下的实现（尤其是 `extractor/` 和命令行参数定义），必要时同时参考同一目录中的 README、FAQ、变更记录和站点说明。
- 不要仅凭记忆、旧版本经验或第三方文章声称某项能力存在。若本地源码没有对应提取器或 URL 规则，应明确说明“不支持直接处理”，不要把推测写成结论。
- 本地已安装的 `yt-dlp` CLI 可用于辅助核验；源码与已安装版本不一致时，分别说明两者版本和结论，不要混淆。
...
- 以本地 `yt-dlp/yt_dlp/extractor/tiktok.py` 中的实际实现为准。
- 当前 `DouyinIE` 只匹配 `https://www.douyin.com/video/<数字ID>` 形式的单视频 URL；不能因为视频元数据含有 `/user/...` 链接，就声称用户主页可以直接作为播放列表处理。
- 当前源码中的 `TikTokUserIE` 针对 `tiktok.com/@...`，不能当作抖音用户主页提取器使用。对 `https://www.douyin.com/user/...` 的“下载全部作品”支持，必须先在源码中找到明确的 Douyin 用户列表实现，否则应说明需要外部生成视频 URL 列表后再批量下载。
...
- 只做用户要求的最小范围修改，保持上游 yt-dlp 源码和既有目录结构不变。
