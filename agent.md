...
- 主页刷新支持 `all`、`week`、`month`、`quarter`、`half_year`、`year` 时间范围；范围必须持久化到刷新和任务记录，并在枚举分页时按作品发布时间过滤，达到截止日期后停止继续翻页。时间范围过滤不能把缺少发布时间的作品静默当作已过期；有限时间范围只更新范围内发现结果，不得把范围外的历史作品标记为 `remote_missing`，只有 `all` 刷新才执行全量远端消失判定。
