...
class JobDB:
    """SQLite persistence for profiles, refresh snapshots and download jobs.

    The schema is intentionally recreated for the new profile-management model;
    no migration from the original prototype tables is attempted.
    """
    ...
    def __init__(self, path: Path):
        ...
        with self._conn:
            existing_jobs = self._conn.execute("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'jobs'").fetchone()
            if existing_jobs:
                columns = {row[1] for row in self._conn.execute("PRAGMA table_info(jobs)").fetchall()}
                if "kind" not in columns:
                    # The prototype schema is intentionally not migrated.
                    self._conn.executescript("DROP TABLE IF EXISTS items; DROP TABLE IF EXISTS jobs;")
    ...
    def create_profile(self, sec_user_id: str, profile_url: str, display_name: str | None = None) -> sqlite3.Row:
        ...
        now = utc_now()
        self._execute(
            "INSERT INTO profiles (id, sec_user_id, profile_url, display_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
            (profile_id, sec_user_id, profile_url, display_name, now, now),
        )
    ...
    def set_profile_refresh(self, profile_id: str, status: str, error: str | None = None) -> None:
        self._execute(
            "UPDATE profiles SET last_refresh_status = ?, last_refresh_error = ?, last_refresh_at = CASE WHEN ? IN ('pending_confirmation', 'applied', 'failed', 'cancelled') THEN ? ELSE last_refresh_at END, updated_at = ? WHERE id = ?",
            (status, error, status, utc_now(), utc_now(), profile_id),
        )

    def profile_stats(self, profile_id: str) -> dict[str, int]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT download_status, COUNT(*) AS count FROM profile_posts WHERE profile_id = ? GROUP BY download_status",
                (profile_id,),
            ).fetchall()
    ...
    def active_refresh(self, profile_id: str) -> sqlite3.Row | None:
        with self._lock:
            return self._conn.execute(
                "SELECT r.* FROM profile_refreshes r JOIN jobs j ON j.id = r.job_id WHERE r.profile_id = ? AND r.status IN ('queued', 'enumerating') AND j.status IN ('queued', 'enumerating') ORDER BY r.created_at DESC LIMIT 1",
                (profile_id,),
            ).fetchone()
    ...
    def pending_refresh(self, profile_id: str) -> sqlite3.Row | None:
        with self._lock:
            return self._conn.execute(
                "SELECT * FROM profile_refreshes WHERE profile_id = ? AND status = 'pending_confirmation' ORDER BY created_at DESC LIMIT 1",
                (profile_id,),
            ).fetchone()
    ...
    def list_refresh_items(self, refresh_id: str) -> list[sqlite3.Row]:
        with self._lock:
            return self._conn.execute(
                "SELECT * FROM profile_refresh_items WHERE refresh_id = ? ORDER BY created_at, aweme_id",
                (refresh_id,),
            ).fetchall()

    def add_refresh_items(self, refresh_id: str, items: list[dict[str, Any]]) -> None:
        self._many(
            "INSERT OR REPLACE INTO profile_refresh_items (refresh_id, aweme_id, title, upload_date, aweme_type, video_url, change_type, is_selected, skip_reason, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)",
            [
                (
                    refresh_id, item["aweme_id"], item.get("title"), item.get("upload_date"),
                    item.get("aweme_type"), item["video_url"], item["change_type"],
    ...
    def complete_refresh(self, refresh_id: str, counts: dict[str, int], status: str = "pending_confirmation", error: str | None = None) -> None:
        ...
        self._execute(
            "UPDATE profile_refreshes SET status = ?, discovered_count = ?, new_count = ?, changed_count = ?, missing_count = ?, skipped_count = ?, error = ?, completed_at = ? WHERE id = ?",
            (status, counts.get("discovered", 0), counts.get("new", 0), counts.get("changed", 0), counts.get("missing", 0), counts.get("skipped", 0), error, completed, refresh_id),
        )
    ...
    def apply_refresh(self, refresh_id: str, selected_ids: list[str]) -> sqlite3.Row:
        ...
        with self._lock:
            ...
            for row in items:
                ...
                existing = self._conn.execute("SELECT download_status FROM profile_posts WHERE profile_id = ? AND aweme_id = ?", (refresh["profile_id"], row["aweme_id"])).fetchone()
                if existing and existing["download_status"] == "downloaded":
                    status = "downloaded"
                self._conn.execute(
                    "INSERT INTO profile_posts (profile_id, aweme_id, title, upload_date, aweme_type, video_url, remote_state, download_status, skip_reason_code, skip_reason_message, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?) ON CONFLICT(profile_id, aweme_id) DO UPDATE SET title=excluded.title, upload_date=excluded.upload_date, aweme_type=excluded.aweme_type, video_url=excluded.video_url, remote_state='active', download_status=CASE WHEN profile_posts.download_status='downloaded' THEN 'downloaded' ELSE excluded.download_status END, skip_reason_code=excluded.skip_reason_code, skip_reason_message=excluded.skip_reason_message, updated_at=excluded.updated_at",
                    (refresh["profile_id"], row["aweme_id"], row["title"], row["upload_date"], row["aweme_type"], row["video_url"], status, skip_code, skip_message, now),
                )
            self._conn.execute("UPDATE profile_refresh_items SET is_selected = CASE WHEN aweme_id IN ({}) THEN 1 ELSE is_selected END WHERE refresh_id = ?".format(",".join("?" * len(selected)) if selected else "SELECT NULL"), (*selected, refresh_id) if selected else (refresh_id,))
    ...
    def add_job_items(self, job_id: str, items: list[dict[str, Any]]) -> None:
        self._many(
            "INSERT OR REPLACE INTO job_items (job_id, aweme_id, video_url, title, status, error_code, error_message, skip_reason_code, skip_reason_message, attempt_count, file_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                (job_id, item["aweme_id"], item.get("video_url"), item.get("title"), item["status"], item.get("error_code"), item.get("error_message"), item.get("skip_reason_code"), item.get("skip_reason_message"), item.get("attempt_count", 0), item.get("file_name"))
                for item in items
            ],
        )
    ...
    def list_jobs(self, limit: int = 100) -> list[sqlite3.Row]:
        ...
        with self._lock:
            return self._conn.execute(
                """
                SELECT j.*, p.display_name, p.sec_user_id
                FROM jobs j
    ...
    def delete_job(self, job_id: str) -> bool:
        with self._lock:
            ...
            if row["status"] in {"queued", "enumerating", "downloading"}:
                return False
            # A refresh snapshot belongs to its job. Remove it together with
            # the terminal job so a deleted pending snapshot cannot block the
            # next refresh for the profile.
    ...
    def update_job_counts(self, job_id: str, **overrides: int) -> None:
        if overrides:
            ...
            return
        with self._lock:
            row = self._conn.execute("SELECT status, COUNT(*) AS n FROM job_items WHERE job_id = ? GROUP BY status", (job_id,)).fetchall()
        counts = {str(item["status"]): int(item["n"]) for item in row}
        self._execute(
            "UPDATE jobs SET discovered = ?, queued = ?, completed = ?, skipped = ?, failed = ?, updated_at = ? WHERE id = ?",
            (sum(counts.values()), counts.get("queued", 0) + counts.get("downloading", 0), counts.get("downloaded", 0), counts.get("skipped", 0) + counts.get("already_downloaded", 0), counts.get("failed", 0), utc_now(), job_id),
        )
    ...
    def update_post(self, profile_id: str, aweme_id: str, **fields: Any) -> None:
        allowed = {key: value for key, value in fields.items() if key in {"download_status", "download_file", "downloaded_at", "attempt_count", "last_attempt_at", "last_error_code", "last_error_message", "skip_reason_code", "skip_reason_message", "remote_state"}}
        if not allowed:
            return
        assignments = ", ".join(f"{key} = ?" for key in allowed)
    ...
    def recover_running(self) -> list[str]:
        with self._lock:
            rows = self._conn.execute("SELECT id, profile_id, created_at FROM jobs WHERE status IN ('queued', 'enumerating', 'downloading') ORDER BY created_at").fetchall()
            keep_profiles: set[str] = set()
            keep_ids: list[str] = []
            for row in rows:
                profile_id = row["profile_id"]
                if profile_id and profile_id in keep_profiles:
                    self._conn.execute("UPDATE jobs SET status = 'cancelled', phase = 'cancelled', error = '同一主页已有刷新任务，重复任务已取消', updated_at = ? WHERE id = ?", (utc_now(), row["id"]))
                    self._conn.execute("UPDATE profile_refreshes SET status = 'cancelled', error = '同一主页已有刷新任务，重复任务已取消', completed_at = ? WHERE job_id = ?", (utc_now(), row["id"]))
                else:
                    if profile_id:
                        keep_profiles.add(profile_id)
                    keep_ids.append(row["id"])
            self._conn.execute("UPDATE jobs SET status = 'queued', phase = 'queued', current_item = NULL, updated_at = ? WHERE status IN ('enumerating', 'downloading')", (utc_now(),))
            self._conn.commit()
            return keep_ids
